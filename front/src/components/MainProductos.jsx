...
const MainProductos = () => {
    ...
    const postProductos = async () => {
        try {
          const response = await axios.post("http://localhost:3000/productos", {
            idProvedor, nombre, descripcion, precio, stock, url
          });
      ...
      const putEditar= async()=>{
        try {
          const response = await axios.put("http://localhost:3000/productos",{
            id,idProvedor, nombre, descripcion, precio, stock, url
          })
          if(response.status === 200){
            //sucessClick(2)
      ...
      const cancelarTurno = (dato)=>{
        axios.delete(`http://localhost:3000/productos/`+dato.id)
        .then((response)=>{
            if(response.status===200){
                //sucessClick(3)
    ...
    return (
      <div>
        <div>
            {switchEditar == false ?(<h4>Agregar Productos</h4>):(<h4>Editar Productos</h4>)}
            ...
            {switchEditar == false ? (
                  <button onClick={postProductos} className="agregar-paciente-btn">Agregar Productos</button>
                ) : (
                  <div>
                    <button onClick={putEditar} className="agregar-paciente-btn">Editar Productos</button>
        ...
        <div className="card-container">
          {datos.length > 0 ? (
            datos.map((dato) => (
              <Card key={dato.id} style={{ width: '18rem' }}>
                <Card.Img variant="top" src={dato.url} />
                <Card.Body>
                  <Card.Title>{dato.nombre}</Card.Title>
                  <Card.Text>{dato.descripcion}</Card.Text>
                  <Card.Text>Precio: {dato.precio}</Card.Text>
                  <Card.Text>Stock: {dato.stock}</Card.Text>
                  <Button onClick={()=>editarProducto(dato)} variant="primary">Editar</Button>
                  <Button onClick={()=>cancelarTurno(dato)} >Eliminar</Button>
                </Card.Body>
              </Card>
            ))
          ) : (
